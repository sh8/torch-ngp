...
class OrbitCamera:
    ...
    def orbit(self, dx, dy):
        ...
        )[:3, 0]  # why this is side --> ? # already normalized.
    ...
    def pan(self, dx, dy, dz=0):
        ...
        self.center += 0.001 * self.rot.as_matrix()[:3, :3] @ np.array(
            [dx, dy, dz])
...
class NeRFGUI:
    ...
    def test_step(self):
        ...
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            outputs = self.trainer.test_gui(self.cam.pose, self.cam.intrinsics,
                                            self.W, self.H, self.bg_color,
                                            self.spp, self.downscale)
            ...
            if self.need_update:
                ...
                self.render_buffer = (self.render_buffer * self.spp +
                                      outputs['image']) / (self.spp + 1)
                self.spp += 1
    ...
    def register_dpg(self):
                ...
                with dpg.collapsing_header(label="Train", default_open=True):
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        def callback_save(sender, app_data):
                            ...
                            dpg.set_value(
                                "_log_ckpt", "saved " + os.path.basename(
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                def callback_change_bg(sender, app_data):
                    self.bg_color = torch.tensor(
                        app_data[:3],
                        dtype=torch.float32)  # only need RGB in [0, 1]
                    self.need_update = True

                dpg.add_color_edit((255, 255, 255),
                ...
                dpg.add_slider_int(label="FoV (vertical)",
                                   min_value=1,
                                   max_value=120,
                                   format="%d deg",
                ...
                dpg.add_slider_float(label="dt_gamma",
                                     min_value=0,
                                     max_value=0.1,
                                     format="%.5f",
        ...
        dpg.create_viewport(title='torch-ngp',
                            width=self.W,
                            height=self.H,
                            resizable=False)
