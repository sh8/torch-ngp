    ...
    def run_cuda(self,
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(
                rays_o, rays_d, self.bound, self.density_grid,
        ...
        for xi, xs in enumerate(X):
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    world_xyzs = torch.cat([
                        xx.reshape(-1, 1),
                        yy.reshape(-1, 1),
                        zz.reshape(-1, 1)
                    ],
                                           dim=-1).unsqueeze(0).to(
                                               count.device)  # [1, N, 3]
                    ...
                    for cas in range(self.cascade):
                        ...
                        while head < B:
                            ...
                            cam_xyzs = cas_world_xyzs - poses[head:tail, :3,
                                                              3].unsqueeze(1)
                            cam_xyzs = cam_xyzs @ poses[
                                head:tail, :3, :3].transpose(1, 2)  # [S, N, 3]
                            ...
                            # update count
                            count[cas, xi * S:xi * S + lx, yi * S:yi * S + ly,
                                  zi * S:zi * S + lz] += mask
                            head += S
            ...
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    for cas in range(self.cascade):
                        ...
                        # assign
                        tmp_grid[cas, xi * S:xi * S + lx, yi * S:yi * S + ly,
                                 zi * S:zi * S + lz] = sigmas
