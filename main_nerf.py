...
from nerf.utils import (seed_everything, Trainer, PSNRMeter, optim)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=0,
        help=
        "num steps up-sampled per ray (only valid when not using --cuda_ray)")
    parser.add_argument(
        '--max_ray_batch',
        type=int,
        default=4096,
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            train_loader = NeRFDataset(opt, device=device,
                                       type='train').dataloader()
            valid_loader = NeRFDataset(opt,
                                       device=device,
                                       type='val',
                                       downscale=2).dataloader()
