...
class _grid_encode(Function):
    ...
    def forward(ctx,
        ...
        _backend.grid_encode_forward(inputs, embeddings, offsets, outputs, B,
                                     D, C, L, S, H, calc_grad_inputs, dy_dx,
                                     gridtype)
    ...
    def backward(ctx, grad):
        ...
        if calc_grad_inputs:
            ...
            grad_inputs = torch.zeros(1,
                                      device=inputs.device,
                                      dtype=inputs.dtype)

        _backend.grid_encode_backward(grad, inputs, embeddings, offsets,
                                      grad_embeddings, B, D, C, L, S, H,
...
class GridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_levels):
            ...
            params_in_level = min(self.max_params,
                                  (resolution +
                                   1)**input_dim)  # limit max number
    ...
    def forward(self, inputs, bound=1):
        ...
        outputs = grid_encode(inputs, self.embeddings, self.offsets,
                              self.per_level_scale, self.base_resolution,
                              inputs.requires_grad, self.gridtype_id)
